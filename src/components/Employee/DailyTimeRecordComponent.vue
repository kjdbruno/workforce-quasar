<template>
    ...
    <q-card-actions class="q-pa-lg">
        <div class="q-gutter-sm">
            <q-btn unelevated size="md" color="primary" class="btn text-capitalize" label="print" @click="PrintDTR" :loading="EmployeeStore.loading" >
...
const LoadTimeCard = async () => {
    ...
    try {
        const { data } = await api.get(`/employee/dtr/${EmployeeStore.profileId}?month=${month.value}&year=${year.value}`);
...
const PrintDTR = async () => {
    EmployeeStore.loading = true;
    try {
        const res = await api.get(`/employee/dtr/${EmployeeStore.profileId}/${month.value}/${year.value}/pdf`, {
