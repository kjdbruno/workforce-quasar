        ...
        <q-dialog v-model="dialog" full-height position="right" persistent square class="dialog">
            <q-card class="dialog-card column full-height">
                <q-card-section class="q-pa-lg">
                    <div class="text-h6 text-uppercase">{{ isEdit ? 'modify premium pay' : 'create new premium pay' }}</div>
                </q-card-section>
                <q-separator inset />
                <q-card-section class="col q-pa-lg scroll">
                    <div class="row q-col-gutter-xs q-mb-md">
                        <div class="col-3">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">premium pay name</span>
                                <q-icon
...
const LoadAll = async () => {
    loading.value = true;
    try {
        const { data } = await api.get(`/premiumpay`, {
            params: { 
                Page: page.value, 
                Limit: limit.value,
                Filter: filter.value || ''
...
const Save = async () => {
    ...
    try {
        const response = id.value && isEdit
            ? await api.post(`/premiumpay/${id.value}/update`, {
                name: name.value
            })
            : await api.post('/premiumpay', {
                name: name.value
            });
        dialog.value = false;
        if (id.value && isEdit) {
            UpdateList(response.data.premiumpay);
        } else {
            LoadAll();
        }
        Toast.fire({
...
const Toggle = async () => {
    ...
    try {
        const response = isActive.value
            ? await api.post(`/premiumpay/${id.value}/disable`)
            : await api.post(`/premiumpay/${id.value}/enable`)
        dialog.value = false;
        UpdateList(response.data.premiumpay)
        Toast.fire({
            icon: "success",
