        ...
        <q-dialog v-model="dialog" full-height position="right" persistent square>
            <q-card class="dialog-card column full-height">
                <q-card-section class="q-pa-lg">
                    <div class="text-h6 text-uppercase">{{ isEdit ? 'modify school level' : 'create new school level' }}</div>
                </q-card-section>
                <q-separator inset />
                <q-card-section class="col q-pa-lg scroll">
                    <div class="row q-col-gutter-xs q-mb-md">
                        <div class="col-3">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">school level name</span>
                                <q-icon
                    ...
                    <div class="row q-col-gutter-xs q-mb-md">
                        <div class="col-3">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">description</span>
                                <q-icon
...
const LoadAll = async () => {
    ...
    try {
        ...
        const { data } = await api.get(`/schoollevel`, {
            params: { 
                Page: page.value, 
                Limit: limit.value,
                Filter: filter.value || ''
...
const Save = async () => {
    ...
    try {
        const response = id.value && isEdit
            ? await api.post(`/schoollevel/${id.value}/update`, {
                name: name.value,
                description: description.value
            })
            : await api.post('/schoollevel', {
                name: name.value,
                description: description.value
            });
        dialog.value = false;
        if (id.value && isEdit) {
            UpdateList(response.data.schoollevel);
        } else {
            LoadAll();
        }
...
const Toggle = async () => {
    ...
    try {
        const response = isActive.value
            ? await api.post(`/schoollevel/${id.value}/disable`)
            : await api.post(`/schoollevel/${id.value}/enable`)
        dialog.value = false;
        UpdateList(response.data.schoollevel)
        Toast.fire({
            icon: "success",
