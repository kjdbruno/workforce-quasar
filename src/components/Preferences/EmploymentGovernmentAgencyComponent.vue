        ...
        <q-dialog v-model="dialog" full-height position="right" persistent square class="dialog">
            <q-card class="dialog-card column full-height">
                <q-card-section class="q-pa-lg">
                    <div class="text-h6 text-uppercase">{{ isEdit ? 'modify government agency' : 'create new government agency' }}</div>
                </q-card-section>
                <q-separator inset />
                <q-card-section class="col q-pa-lg scroll">
                    <div class="row q-col-gutter-xs q-mb-md">
                        <div class="col-3">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">government name</span>
                                <q-icon
                        ...
                        <div class="col-1">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">alias</span>
                                <q-icon
...
const LoadAll = async () => {
    loading.value = true;
    try {
        const { data } = await api.get(`/governmentagency`, {
            params: { 
                Page: page.value, 
                Limit: limit.value,
                Filter: filter.value || ''
...
const Save = async () => {
    ...
    try {
        const response = id.value && isEdit
            ? await api.post(`/governmentagency/${id.value}/update`, {
                name: name.value,
                alias: alias.value
            })
            : await api.post('/governmentagency', {
                name: name.value,
                alias: alias.value
            });
        dialog.value = false;
        if (id.value && isEdit) {
            UpdateList(response.data.governmentagency);
        } else {
            LoadAll();
        }
        Toast.fire({
...
const Toggle = async () => {
    ...
    try {
        const response = isActive.value
            ? await api.post(`/governmentagency/${id.value}/disable`)
            : await api.post(`/governmentagency/${id.value}/enable`)
        dialog.value = false;
        UpdateList(response.data.governmentagency)
        Toast.fire({
            icon: "success",
