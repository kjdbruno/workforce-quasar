            ...
            >
                <q-card
                    ...
                    <q-card-section class="text-center full-width">
                        <div class="text-subtitle2 text-uppercase">{{ data?.positions?.name }}</div>
                    </q-card-section>
        ...
        <q-dialog v-model="dialog" full-height position="right" persistent square class="dialog">
            <q-card class="dialog-card column full-height">
                <q-card-section class="q-pa-lg">
                    <div class="text-h6 text-uppercase">{{ isEdit ? 'modify salary' : 'create new salary' }}</div>
                </q-card-section>
                <q-separator inset />
                <q-card-section class="col q-pa-lg scroll">
                    <div class="row q-col-gutter-xs q-mb-md">
                        <div class="col-2">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">select position</span>
                                <q-icon
                        ...
                        <div class="col-2">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">select salary class</span>
                                <q-icon
                            ...
                            <q-select
                                ...
                                use-input
                                input-debounce="300"
                                :options="filteredSalaryClasses"
                                @filter="filterSalaryClassFn"
                        ...
                        <div class="col-2">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">select salary grade</span>
                                <q-icon
...
const ModifyDialog = async (data) => {
    ...
    isEdit.value = true;
    id.value = data.id;
    positionId.value = Number(data.positionId);
    classId.value = Number(data.classId);
    gradeId.value = Number(data.gradeId);
...
const Save = async () => {
    ...
    try {
        const response = id.value && isEdit
            ? await api.post(`/salary/${id.value}/update`, {
        ...
        dialog.value = false;
        if (id.value && isEdit) {
            UpdateList(response.data.salary);
        } else {
            LoadAll();
        }
        Toast.fire({
...
const Toggle = async () => {
    ...
    try {
        const response = isActive.value
            ? await api.post(`/salary/${id.value}/disable`)
            : await api.post(`/salary/${id.value}/enable`)
        dialog.value = false;
        UpdateList(response.data.salary)
        Toast.fire({
            icon: "success",
...
const filteredSalaryClasses = ref([]);
const filteredSalaryGrades = ref([]);
const filterPositionFn = createFilterFn(positions, filteredPositions);
const filterSalaryClassFn = createFilterFn(salaryclasses, filteredSalaryClasses);
const filterGradeFn = createFilterFn(salarygrades, filteredSalaryGrades);
...
const LoadPositions = async () => {
    try {
        const { data } = await api.get(`/option/positions`);
...
const LoadSalaryClasses = async () => {
    try {
        const { data } = await api.get(`/option/salaryclasses`);
        salaryclasses.value = normalizeOptions(data)
        filteredSalaryClasses.value = [...salaryclasses.value]
    } catch (error) {
        console.error("Error fetching options:", error);
    }
};
...
const LoadSalaryGrades = async () => {
    try {
        const { data } = await api.get(`/option/salarygrades`);
        salarygrades.value = normalizeOptions(data)
        filteredSalaryGrades.value = [...salarygrades.value]
    } catch (error) {
        console.error("Error fetching options:", error);
    }
};
