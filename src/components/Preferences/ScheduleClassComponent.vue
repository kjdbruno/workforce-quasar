        ...
        <q-dialog v-model="dialog" full-height position="right" persistent square class="dialog">
            <q-card class="dialog-card column full-height">
                <q-card-section class="q-pa-lg">
                    <div class="text-h6 text-uppercase">{{ isEdit ? 'modify schedule class' : 'create new schedule class' }}</div>
                </q-card-section>
                <q-separator inset />
...
const LoadAll = async () => {
    loading.value = true;
    try {
        const { data } = await api.get(`/scheduleclass`, {
            params: { 
                Page: page.value, 
                Limit: limit.value,
                Filter: filter.value || ''
...
const Save = async () => {
    ...
    try {
        const response = id.value && isEdit
            ? await api.post(`/scheduleclass/${id.value}/update`, {
                name: name.value
            })
            : await api.post('/scheduleclass', {
                name: name.value
            });
        dialog.value = false;
        if (id.value && isEdit) {
            UpdateList(response.data.scheduleclass);
        } else {
            LoadAll();
        }
        Toast.fire({
...
const Toggle = async () => {
    ...
    try {
        const response = isActive.value
            ? await api.post(`/scheduleclass/${id.value}/disable`)
            : await api.post(`/scheduleclass/${id.value}/enable`)
        dialog.value = false;
        UpdateList(response.data.scheduleclass)
        Toast.fire({
            icon: "success",
