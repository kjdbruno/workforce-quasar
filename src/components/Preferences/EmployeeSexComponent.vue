        ...
        <q-dialog v-model="dialog" full-height position="right" persistent square class="dialog">
            <q-card class="dialog-card column full-height">
                <q-card-section class="q-pa-lg">
                    <div class="text-h6 text-uppercase">{{ isEdit ? 'modify sex' : 'create new sex' }}</div>
                </q-card-section>
                <q-separator inset />
                <q-card-section class="col q-pa-lg scroll">
                    <div class="row q-col-gutter-xs q-mb-md">
                        <div class="col-3">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">sex name</span>
                                <q-icon
...
const Save = async () => {
    ...
    try {
        const response = id.value && isEdit
            ? await api.post(`/sex/${id.value}/update`, {
                name: name.value
            })
            : await api.post('/sex', {
                name: name.value
...
const Toggle = async () => {
    ...
    try {
        const response = isActive.value
            ? await api.post(`/sex/${id.value}/disable`)
            : await api.post(`/sex/${id.value}/enable`)
        dialog.value = false;
        UpdateList(response.data.sex)
        Toast.fire({
            icon: "success",
