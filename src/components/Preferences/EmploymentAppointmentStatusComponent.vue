        ...
        <q-dialog v-model="dialog" full-height position="right" persistent square class="dialog">
            <q-card class="dialog-card column full-height">
                <q-card-section class="q-pa-lg">
                    <div class="text-h6 text-uppercase">{{ isEdit ? 'modify appointment status' : 'create new appointment status' }}</div>
                </q-card-section>
                <q-separator inset />
                <q-card-section class="col q-pa-lg scroll">
                    <div class="row q-col-gutter-xs q-mb-md">
                        <div class="col-3">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">appointment name</span>
                                <q-icon
                    ...
                    <div class="row q-col-gutter-xs q-mb-md">
                        <div class="col-3">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">appointment description</span>
                                <q-icon
...
const Validations = () => {
    ...
    if (!description.value) {
        ...
        Errors.description.messages.push('description is required')
        isError = true
    } else {
...
const LoadAll = async () => {
    loading.value = true;
    try {
        const { data } = await api.get(`/appointmentstatus`, {
            params: { 
                Page: page.value, 
                Limit: limit.value,
                Filter: filter.value || ''
...
const Save = async () => {
    ...
    try {
        const response = id.value && isEdit
            ? await api.post(`/appointmentstatus/${id.value}/update`, {
                name: name.value,
                description: description.value
            })
            : await api.post('/appointmentstatus', {
                name: name.value,
                description: description.value
            });
        dialog.value = false;
        if (id.value && isEdit) {
            UpdateList(response.data.appointmentstatus);
        } else {
            LoadAll();
        } 
        Toast.fire({
...
const Toggle = async () => {
    ...
    try {
        const response = isActive.value
            ? await api.post(`/appointmentstatus/${id.value}/disable`)
            : await api.post(`/appointmentstatus/${id.value}/enable`)
        dialog.value = false;
        UpdateList(response.data.appointmentstatus)
        Toast.fire({
            icon: "success",
