        ...
        <q-dialog v-model="dialog" full-height position="right" persistent square>
            <q-card class="dialog-card column full-height">
                <q-card-section class="q-pa-lg">
                    <div class="text-h6 text-uppercase">{{ isEdit ? 'modify increment' : 'create new increment' }}</div>
                </q-card-section>
                <q-separator inset />
                <q-card-section class="col q-pa-lg scroll">
                    <div class="row q-col-gutter-xs q-mb-xs">
                        <div class="col-3">
                            <div class="q-mb-xs">
                                <span class="text-caption text-uppercase text-grey q-mr-sm">increment name</span>
                                <q-icon
...
const LoadAll = async () => {
    loading.value = true;
    try {
        const { data } = await api.get(`/increment`, {
            params: { 
                Page: page.value, 
                Limit: limit.value,
                Filter: filter.value || ''
...
const Save = async () => {
    ...
    try {
        const response = id.value && isEdit
            ? await api.post(`/increment/${id.value}/update`, {
                name: name.value
            })
        ...
        dialog.value = false;
        if (id.value && isEdit) {
            UpdateList(response.data.increment);
        } else {
            LoadAll();
        }
        Toast.fire({
...
const Toggle = async () => {
    ...
    try {
        const response = isActive.value
            ? await api.post(`/increment/${id.value}/disable`)
            : await api.post(`/increment/${id.value}/enable`)
        dialog.value = false;
        UpdateList(response.data.increment)
        Toast.fire({
            icon: "success",
