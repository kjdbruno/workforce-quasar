<template>
    <div>
        ...
        <q-dialog v-model="dialog" full-height position="right" persistent square class="dialog">
            <q-card class="dialog-card column full-height">
                ...
                <q-card-section class="col q-pa-lg scroll">
                    <div class="q-mb-xl" v-if="applications.length">
                        <div class="q-mb-sm">
        ...
        <q-dialog v-model="DetailDialog" full-height position="right" persistent square class="dialog">
            <q-card class="dialog-card column full-height">
                <q-card-section class="q-pa-lg">
                    <div class="text-h6 text-uppercase">employee information</div>
                </q-card-section>
                <q-separator inset />
                <q-card-section class="col q-pa-lg scroll">
                    ...
                    <div class="q-mt-lg">
                        <component :is="components[EmployeeStore.subComponent || 'EmployeeProfileComponent']" />
                    </div>
                </q-card-section>
...
const LoadSalaryPositions = async () => {
    try {
        const { data } = await api.get(`/option/salarypositions`);
        salaries.value = data
    } catch (error) {
        console.error("Error fetching options:", error);
    }
...
import EmployeeProfileComponent from './ProfileComponent.vue';
import EmployeeEmploymentComponent from './EmploymentComponent.vue';
import EmployeeScheduleComponent from './ScheduleComponent.vue';
import EmployeePhotoComponent from './PhotoComponent.vue';
import EmployeeEducationComponent from './EducationComponent.vue';
import EmployeeTrainingsComponent from './TrainingComponent.vue';
import EmployeeExperiencesComponent from './ExperienceComponent.vue';
import EmployeeDependentsComponent from './DependentComponent.vue';
import EmployeeDocumentsComponent from './DocumentComponent.vue';
import EmployeeDigitalIDComponent from './IDComponent.vue';
import ServiceRecordComponent from './ServiceRecordComponent.vue';
import LeavesComponent from './LeavesComponent.vue';
import DailyTimeRecordComponent from './DailyTimeRecordComponent.vue';
...
const activeTab = ref(EmployeeStore.activeTab || "Employee");
const activeSubTab = ref(EmployeeStore.activeSubTab || "Profile");
const dropdownOpen = ref(null);
const DetailDialog = ref(false);
const DetailSubmitting = ref(false);

function buildComponentName(tab, sub) {
  const t = tab.replace(/\s+/g, "");
  const s = sub?.replace(/\s+/g, "") || "";
  return sub ? `${t}${s}Component` : `${t}Component`;
}


function selectTab(tabName, subTab = null) {
  const isEmployeeWithSub = tabName === "Employee" && subTab;
  
  // set active tab/subtab
  activeTab.value = tabName;
  activeSubTab.value = isEmployeeWithSub ? subTab : null;

  // compute component
  const componentName = isEmployeeWithSub
    ? buildComponentName(tabName, subTab)
    : buildComponentName(tabName);

  // store fallback logic
  EmployeeStore.activeTab = tabName || "Employee";
  EmployeeStore.activeSubTab = subTab || (tabName === "Employee" ? "Profile" : null);
  EmployeeStore.subComponent = componentName || "EmployeeProfileComponent";
...
const InfoDialog = (data) => {
  ...
  EmployeeStore.employment = data;

  // restore last active tab/subTab
  activeTab.value = EmployeeStore.activeTab;
  activeSubTab.value = EmployeeStore.activeSubTab;
};


watch(DetailDialog, (val) => {
  if (!val) {
    EmployeeStore.isEmployeeProfile = false;
    EmployeeStore.isEmployeeEmployment = false;
    EmployeeStore.isEmployeeEducation = false;
  }
});
...
const HandleSave = () => {
    EmployeeStore.triggerChildSave();
};
...
onBeforeMount(() => {
    ...
    EmployeeStore.isEmployeeEmployment = false;
    EmployeeStore.isEmployeeEducation = false;
    EmployeeStore.isEmployeeTraining = false;
});
</script>
