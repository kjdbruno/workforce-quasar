...
export default boot(({ app }) => {
  ...
  // ✅ register listeners ONCE (not inside connect)
  socket.on('EmitOnlineUsers', (users) => {
    authStore.setOnlineUsers(users); // use your action (recommended)
    console.log('online users:', users);
  });
  ...
  socket.on('connect', () => {
    ...
    if (authStore.isAuthenticated && authStore.token && authStore.user?.id) {
      socket.emit('authenticate', { token: authStore.token });
      socket.emit('register', { token: authStore.token });
      socket.emit('join', authStore.user.id);
      socket.emit('GetOnlineUsers'); // ✅ ask server to send current list (optional)
    }
  });
  ...
  authStore.$subscribe((mutation, state) => {
    if (state.isAuthenticated && state.token && !socket.connected) {
      socket.connect();
      socket.emit('authenticate', { token: state.token });
    } else if (!state.isAuthenticated && socket.connected) {
      socket.disconnect();
    }
  });
